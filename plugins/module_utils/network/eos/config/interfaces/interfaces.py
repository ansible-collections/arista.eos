...
def state_replaced(want, have):
    ...
    for interface, commands in remove.items():
        commands_by_interface[interface] = (
            replace.get(interface, []) + commands
        )
...
def _compute_commands(want, have, replace=False, remove=False):
    ...
    for name, config in want.items():
        ...
        if remove:
            remove_params[name] = dict(
                set(extant.items()).difference(config.items())
            )
        if replace:
            replace_params[name] = dict(
                set(config.items()).difference(extant.items())
            )
            if remove:
        ...
        for param, state in config.items():
            if param == "description":
                ...
                interface_commands.append(
                    "{0}shutdown".format("no " if state else "")
                )
            elif param == "mtu":
                interface_commands.append("mtu {0}".format(state))
        if "speed" in config:
            interface_commands.append(
                "speed {0}{1}".format(config["speed"], config["duplex"])
            )
        if interface_commands:
            commands[interface] = interface_commands
